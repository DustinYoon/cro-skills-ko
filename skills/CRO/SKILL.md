...
**아티팩트가 바로 오면**(URL·전사·CSV·파일경로) 인테이크 없이 해당 스킬의 **아티팩트 우선 진입**으로 넘긴다(통화/딜→`CRO-deal`, 파이프라인 export→`CRO-forecast`, 경쟁사/계정 리스트→`CRO-market`). 사용자는 `/cro-deal <url>`처럼 스킬을 직접 부를 수도 있다.

## 2.5 병렬 처리 (fan-out — 진단 유닛이 많을 때만)
- **언제:** (a) **독립적인 스킬 진단이 2개 이상**(예: 조직+딜+시장 동시 점검)이거나, (b) 한 스킬 안에서 처리할 **유닛이 많을 때**(딜 >8·세그먼트 >3) — 후자는 그 스킬(`CRO-forecast`/`CRO-market`)이 자체 fan-out으로 처리하니 **거기로 위임**한다. 유닛이 적으면 인라인(오버헤드 낭비 금지).
- **어떻게:** `Task` 툴로 진단 축마다 서브에이전트 1개를 병렬 실행. 각 서브에이전트에 **해당 CRO-* 스킬을 로드하도록 지시**하고, **데이터 소스 규율·수치 날조 금지**를 그대로 상속시킨다. 반환은 축별 `{축·소스줄·판정·최상위 액션}`으로 구조화.
- **집계:** 축별 소스줄을 보존한 채 하나의 진단으로 통합하고 **상충은 뭉개지 말고 `conflict-resolution.md` §3으로 조건별로 드러낸다**. 서브에이전트가 못 채운 칸은 '다음 질문'으로(집계가 빈칸을 지어내지 않음).
