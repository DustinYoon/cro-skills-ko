...
| `hooks.json` | 플러그인으로 패키징할 때 쓰는 훅 설정(matcher: `startup\|clear\|compact`) |

## 설치

### 방법 1 — `install.sh` (권장, 로컬 클론에서)

리포지토리 루트의 `install.sh`가 스킬을 설치하면서 이 훅을 처리한다:

1. `hooks/`를 **`~/.claude/cro-hooks/`로 복사**한다(클론을 옮기거나 지워도 훅이 계속 동작 — self-contained).
2. `~/.claude/settings.json`에 그 복사본 경로를 **멱등적으로** 병합한다(이미 있으면 건너뜀).
   기존 설정이 손상(JSON 파싱 불가)돼 있으면 덮어쓰지 않고 `*.corrupt.bak.<stamp>`로 백업한 뒤 중단하며,
   성공 시에도 `settings.json.bak.<stamp>`를 남기고 임시파일 원자적 치환으로 쓴다.
...
> **`npx skills`로 설치한 경우:** `npx skills add …`는 `skills/`만 복사하고 `hooks/`는 가져오지 않는다.
> 즉 **자동 발화 훅은 설치되지 않는다.** 로컬 클론에서 `bash install.sh`를 한 번 돌리거나, 아래 방법 2로
> 수동 등록해야 자동 발화가 켜진다.

### 방법 2 — 수동으로 `~/.claude/settings.json`에 추가
...
스위트를 Claude Code 플러그인으로 배포하면 `hooks.json`이 그대로 쓰인다(`${CLAUDE_PLUGIN_ROOT}` 치환).
